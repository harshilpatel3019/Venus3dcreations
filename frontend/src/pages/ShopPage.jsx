      ...
      <div className="bg-[var(--ivory)] border-b border-[var(--sand)]">
        <div className="max-w-[1400px] mx-auto px-6 lg:px-10 py-16 lg:py-24">
          ...
          <p className="mt-4 text-[var(--espresso)]/60 max-w-xl">
            {activeCat ? activeCat.tagline : "Browse every object from the studio."}
          </p>
        </div>
      </div>
      ...
      <div className="max-w-[1400px] mx-auto px-6 lg:px-10 py-12">
        <div className="flex flex-wrap items-center justify-between gap-4 mb-10">
          <div className="flex flex-wrap gap-2">
            <Link to="/shop" className={`px-5 h-10 inline-flex items-center text-xs tracking-[0.14em] uppercase border transition-colors ${!category ? "bg-[var(--espresso)] text-white border-[var(--espresso)]" : "border-[var(--sand)] hover:border-[var(--copper)]"}`}>All</Link>
            {categories.map((c) => (
              <Link key={c.id} to={`/shop/${c.id}`} className={`px-5 h-10 inline-flex items-center text-xs tracking-[0.14em] uppercase border transition-colors ${category === c.id ? "bg-[var(--espresso)] text-white border-[var(--espresso)]" : "border-[var(--sand)] hover:border-[var(--copper)]"}`}>{c.name}</Link>
            ))}
          </div>

          <div className="relative">
            <button onClick={() => setSortOpen((v) => !v)} className="inline-flex items-center gap-2 px-5 h-10 border border-[var(--sand)] text-xs tracking-[0.14em] uppercase hover:border-[var(--copper)]">
            ...
            {sortOpen && (
              <div className="absolute right-0 top-12 bg-[var(--cream)] border border-[var(--sand)] shadow-lg z-10 min-w-[220px]">
                {SORTS.map((s) => (
                  <button key={s.id} onClick={() => { setSort(s.id); setSortOpen(false); }} className={`w-full text-left px-4 py-3 text-xs tracking-[0.1em] uppercase hover:bg-[var(--sand)] ${sort === s.id ? "text-[var(--copper)]" : ""}`}>
                    {s.label}
                  </button>
                ))}
              </div>
        ...
        {loading ? (
          ...
          <>
            <p className="text-xs tracking-[0.14em] uppercase text-[var(--espresso)]/50 mb-8">Showing {filtered.length} {filtered.length === 1 ? "piece" : "pieces"}</p>
