...
const CartDrawer = () => {
  ...
  return (
    <>
      ...
      {/* Drawer */}
      <aside
        className={`fixed top-0 right-0 h-full w-full max-w-md bg-[var(--cream)] z-50 shadow-2xl transform transition-transform duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] ${
      ...
      >
        <div className="h-full flex flex-col">
          <div className="flex items-center justify-between px-6 h-20 border-b border-[var(--sand)]">
            ...
            <button onClick={() => setOpen(false)} className="p-2 hover:text-[var(--copper)]">
              <X className="w-5 h-5" />
            </button>
          </div>
          ...
          {items.length === 0 ? (
            <div className="flex-1 flex flex-col items-center justify-center px-6 text-center">
              <div className="w-16 h-16 rounded-full bg-[var(--sand)] flex items-center justify-center mb-5">
            ...
            <>
                  ...
                  <div key={item.id} className="flex gap-4">
                    ...
                    <div className="flex-1 flex flex-col justify-between">
                      <div>
                        ...
                        <p className="text-sm text-[var(--espresso)]/60 mt-1">${item.price.toFixed(2)}</p>
                      </div>
                      <div className="flex items-center justify-between">
                        <div className="flex items-center border border-[var(--sand)]">
                          <button onClick={() => updateQty(item.id, item.qty - 1)} className="w-8 h-8 flex items-center justify-center hover:bg-[var(--sand)]">
                        ...
                        <button onClick={() => removeItem(item.id)} className="text-xs uppercase tracking-widest text-[var(--espresso)]/50 hover:text-[var(--copper)]">
                          Remove
                        </button>
                      </div>
                    </div>
              ...
              <div className="border-t border-[var(--sand)] px-6 py-6 space-y-4">
                <div className="flex justify-between">
                  ...
                  <span className="font-serif-display text-xl">${subtotal.toFixed(2)}</span>
                </div>
                <p className="text-xs text-[var(--espresso)]/50">Shipping and taxes calculated at checkout.</p>
                ...
                <button onClick={() => setOpen(false)} className="w-full text-center text-xs tracking-widest uppercase text-[var(--espresso)]/60 hover:text-[var(--copper)]">
                  Continue shopping
                </button>
              </div>
            </>
